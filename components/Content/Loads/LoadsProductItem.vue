<template>
  <tr draggable="false" :key="prod.length">
    ...
    <td class="is-flex">
      <label for="expiration" class="is-hidden-desktop">{{
        $t('table.expiration')
      }}</label>
      <client-only v-if="prod.expiration">
        <b-input
          size="is-small"
          :value="filterDate(prod.expiration)"
          :disabled="disabled"
          name="expiration"
        ></b-input>
      </client-only>
...
export default {
  ...
  methods: {
    ...
    saveLoadDetail() {
      ...
      let payload = {
        ...
        price: this.prod.product.cost,
        expiration: this.expiration,
        product: {
          id: this.prod.product.id,
          cost_tolerance: this.prod.product.cost_tolerance,
  ...
  computed: {
    totale() {
      ...
      let cost = parseFloat(this.prod.product.cost)
      let tax
      if (this.prod.tax) {
