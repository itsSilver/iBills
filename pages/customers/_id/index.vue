...
export default {
  ...
  methods: {
    async submit() {
      await this.$axios
        .put(`/customers/${this.$route.params.id}`, this.data, {
          headers: {
  ...
  async asyncData({ store, $axios, params, $auth }) {
    let response = await $axios.get(`/customers/${params.id}`, {
      headers: {
