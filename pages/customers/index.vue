...
export default {
  ...
  head() {
    return {
      title: `${this.$t('nav.customers')} — ${this.$t('messages.title')}`,
    }
  },
  data() {
    return {
  ...
  methods: {
    async deleteAllSelected() {
      ...
      await this.$axios
        .post('/customers/deleteAll', payload, {
          headers: {
    ...
    // Update supplier
    updateSupplier(id) {
      this.$router.push(this.localePath(`/customers/${id}`))
    },
    onNewDocument() {
      // this.$store.commit('companies/NEW_COMPANY')
      this.$router.push(this.localePath({ name: 'customers-create' }))
    },
    async loadAsyncData() {
      this.loading = true
      await this.$axios
    ...
    async trashConfirm() {
      this.loading = true
      await this.$axios
        .delete(`/customers/${this.trashObject.id}`, {
          headers: {
            Authorization: `Bearer ${this.$auth.strategy.token.get()}`,
            'Content-Type': 'application/json',
          },
