<template>
  <div>
    ...
    <section class="section is-main-section is-fullheight">
      ...
      <unload-modal-create
        :isModalActive="showCreateModal"
        @cancel="cancelCreateModal"
        :title="$t('messages.create', { name: `${$t('unload.unload')}` })"
      />
    </section>
  </div>
</template>
...
import UnloadsTable from '@/components/Content/Unloads/UnloadsTable.vue'
import UnloadModalCreate from '@/components/Modals/Unloads/UnloadModalCreate.vue'

export default {
  name: 'Unloads',
  head() {
    return {
      title: `${this.$t('nav.unloads')} — ${this.$t('messages.title')}`,
    }
  },
  components: {
    HeroBar,
  ...
  methods: {
    ...
    async trashConfirm() {
      this.loading = true
      await this.$axios
        .delete(`/unloads/${this.trashObject.id}`, {
          headers: {
            Authorization: `Bearer ${this.$auth.strategy.token.get()}`,
            'Content-Type': 'application/json',
          },
    ...
    async deleteAllSelected() {
      ...
      await this.$axios
        .post('/unloads/deleteAll', payload, {
          headers: {
            Authorization: `Bearer ${this.$auth.strategy.token.get()}`,
            'Content-Type': 'application/json',
          },
