  ...
  <div>
    ...
    <section class="section is-main-section is-fullheight">
      ...
      <load-modal-create
        ...
        :title="$t('messages.create', { name: `${$t('load.load')}` })"
      />
    </section>
  </div>
</template>
...
import LoadsTable from '@/components/Content/Loads/LoadsTable.vue'
import LoadModalCreate from '~/components/Modals/Loads/LoadModalCreate.vue'

export default {
  name: 'Loads',
  head() {
    return {
      title: `${this.$t('nav.loads')} — ${this.$t('messages.title')}`,
    }
  },
  components: {
    HeroBar,
  ...
  methods: {
    ...
    async trashConfirm() {
      this.loading = true
      await this.$axios
        .delete(`/loads/${this.trashObject.id}`, {
          headers: {
            Authorization: `Bearer ${this.$auth.strategy.token.get()}`,
            'Content-Type': 'application/json',
          },
