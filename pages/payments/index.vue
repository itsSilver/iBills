  ...
  <div>
    ...
    <section class="section is-main-section is-fullheight">
            ...
            >
              <div class="buttons is-right">
                <button
                  ...
                  type="button"
                  @click.prevent="updatePayment(props.row.id)"
                >
      ...
      <payment-create
        ...
        @payment-created="onPaymentCreate"
        :title="$t('messages.create', { name: `${$t('payment.payment')}` })"
      />
      <payment-update
        :isModalActive="isUpdateModalActive"
        :categoryId="categoryId"
        @cancel="cancelUpdateModal"
        @payment-updated="onPaymentUpdate"
        :title="$t('messages.update', { name: `${$t('payment.payment')}` })"
      />
    </section>
  </div>
</template>
...
import PaymentCreate from '~/components/Modals/Payments/PaymentCreate.vue'
import PaymentUpdate from '~/components/Modals/Payments/PaymentUpdate.vue'
export default {
  name: 'VatRates',
  head() {
    return {
      title: `${this.$t('nav.cash-unit')} — ${this.$t('messages.title')}`,
    }
  },
  components: {
    HeroBar,
  ...
  methods: {
    ...
    async trashConfirm() {
      this.loading = true
      await this.$axios
        .delete(`/payments/${this.trashObject.id}`, {
          headers: {
            Authorization: `Bearer ${this.$auth.strategy.token.get()}`,
            'Content-Type': 'application/json',
          },
