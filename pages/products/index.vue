        ...
        <template #navtop>
          <nav-top-general>
            <button
              ...
              type="button"
              @click="$router.push(localePath('/products/create'))"
              v-role="'admin'"
            >
...
import ProductsTable from '@/components/Content/Products/ProductsTable.vue'
import ImportItems from '~/components/ImportItems.vue'
...
export default {
  ...
  head() {
    return {
      title: `${this.$t('nav.products')} — ${this.$t('messages.title')}`,
    }
  },
  components: {
    HeroBar,
  ...
  methods: {
    ...
    async trashConfirm() {
      this.loading = true
      await this.$axios
        .delete(`/products/${this.trashObject.id}`, {
          headers: {
            Authorization: `Bearer ${this.$auth.strategy.token.get()}`,
            'Content-Type': 'application/json',
          },
    ...
    async deleteAllSelected() {
      ...
      await this.$axios
        .post('/products/deleteAll', payload, {
          headers: {
            Authorization: `Bearer ${this.$auth.strategy.token.get()}`,
            'Content-Type': 'application/json',
          },
